...
$parent = "$pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerSingle"; # directory with the MarkerScanner
...
sub read_files {
	...
	close($fh);
	if($count / $total >= 0.95) {
		system("mv $pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerSingle/$filename $pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerCounted/");
	}
	$count = "0";
}
