...
$parent = "$pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerOutput"; # directory with the coutProteins output
...
sub read_files {
	...
	foreach $genome (@genomes) {
		...
		close($fh);
		if($count == 1) {
			$outputFile = "$pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerSingle/$filename";
			open $out, '>>', $outputFile;
			open $fh, '<', $filename;
			while(<$fh>) {
				if(/>/) {
