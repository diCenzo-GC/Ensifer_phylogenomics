...
# set variables
$species_list = 'Input_lists/genomeList_combined.txt';
$pwd = cwd();
$total = 0;
$count = 0;
...
$parent = "$pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerCounted"; # directory with the coutProteins output
...
sub read_files {
	...
	if($count eq $total) { # this will select the file only if the protein is single copy
		system("mv $pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerCounted/$filename $pwd/Rooted_phylogeny/MLSA_phylogeny/MarkerScannerGood/");
	}
	$total ='0';
	$count ='0';
}
