...
my $AMPHORA_home = $ENV{"HOME"};
my $AMPHORA_home = $AMPHORA_home . '/AMPHORA';
my $MarkerListInput = $AMPHORA_home . '/Marker/marker.list';
my (%markerlist, %seq) = ();
my $help = undef;
my $evalue = 1e-7;
...
# HMM search
system ("hmmsearch -Z 1081916 -E $evalue --domE $evalue --domtblout $$.hmmsearch -o /dev/null $ref_dir"."markers.hmm $input_seq");		# fix the number of sequences in the database for E-value calculation
get_hmm_hits();
...
####################################################################################################################
sub get_marker_list {
	open (IN, $MarkerListInput) || die "Can't open $AMPHORA_home/Marker/marker.list";
	while (<IN>) {
		chop;
		/^(\S+)\s+(\S+)/;
		$markerlist{$1} = $2;
